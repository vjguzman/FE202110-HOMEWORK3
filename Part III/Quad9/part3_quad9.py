...
from quad9 import quad9, quad9_post, quad9_nodal_stress_averaging,quad9_line_load
...
def quad9_partIII(file_name, properties, properties_load):
	...
	for e in Quads:
		...
		if e in placa:
			ke, fe = quad9(xy_e, properties[0])

		if e in extremos:
			ke,fe = quad9(xy_e, properties[1])
	...
	write_node_data2(f"desplazamientos_{file_name}", nodes, uv[:,0], uv[:,1], "Desplazamientos")
...
properties_load["tx"] = 1e3/(properties_load["t"]*9)
properties_load["ty"] = 0


properties = [properties0, properties1]

file_name = ["025_quad9.msh", "075_quad9.msh", "125_quad9.msh"]
for i in file_name:
	print("Empezando...")
	print(f'File: {i}')

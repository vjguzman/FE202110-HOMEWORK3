...
def quad9_partIII(file_name, properties, properties_load):
	...
	for e in Quads:
		...
		if e in placa:
			...
			W = [sigma[0], sigma[1], epsilon[2]]
			stress = quad9_nodal_stress_averaging(xy_e,u_e,properties[0],W)

		if e in extremos:
			epsilon, sigma = quad9_post(xy_e, u_e, properties[1])
			W = [sigma[0], sigma[1], epsilon[2]]
			stress = quad9_nodal_stress_averaging(xy_e,u_e,properties[1],W)
...
for i in file_name:
	print("Empezando...")
	print(f'File: {i}')
	quad9_partIII(i,properties,properties_load)
